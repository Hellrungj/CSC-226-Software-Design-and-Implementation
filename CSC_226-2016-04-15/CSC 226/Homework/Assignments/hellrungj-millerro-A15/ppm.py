...
'''
    ...
    def PPM_flip_horizontal(self):
        ...
        newpixellist=self.pixellist
        # Hint: What needs to be done here is to convert newpixellist to the equivalent horizontally flipped image.
        # The final call to self.PPM_updatefrompixellist(newpixellist) is essential for updating member attribute appropriately.

        # FIX ME: write the needed changes to newpixellist here

        for row in range(len(newpixellist)):
            newpixellist[row].reverse()    #For each row reverse the pixels in that row
    ...
    def PPM_greyscale(self):
        ...
        for rowlist in newpixellist:
            col=0
            for pixel in rowlist:
                pixel = newpixellist[row][col]
                sum_of_colors = 0
                for rgb in pixel:
                    sum_of_colors += (rgb)
                avg = sum_of_colors/3
                newpixellist[row][col][0]=avg
                newpixellist[row][col][1]=avg # update green
                newpixellist[row][col][2]=avg # update blue
                col+=1
            row+=1
        print(self.outasciifile+ " output file turned red.")
    ...
    def PPM_rotateclockwise(self):
        ...
        newpixellist=self.pixellist
        # Hint: What needs to be done here is to convert newpixellist to the equivalent rotated image.
        # The final call to self.PPM_updatefrompixellist(newpixellist) is essential for updating member attribute appropriately.

        # FIX ME: write the needed changes to newpixellist here
        templist = newpixellist
...
##        templist[0][0][2]=0
        ...
        for column in range(columns):
            row_list = [] # Reset the row list after each time we add it to the list of rows in column list.
            for row in range(rows):
                row_list.append(templist[row][column]) # Changes the elements in a single column to a row of the same elements
            #for row in range(len(row_list)):
            row_list.reverse() # However these elements have been put in their respective rows in an opposite order so we fix that
            column_list.append(row_list) # Append it to the column list of rows.
        newpixellist = column_list
        self.PPM_updatefrompixellist(newpixellist) # This call will update all member apttributes appropriately.

    def PPM_flip_colors(self):
        ''''flips image colors'''
        newpixellist=self.pixellist
        # Hint: What needs to be done here is to convert newpixellist to the equivalent horizontally flipped image.
        # The final call to self.PPM_updatefrompixellist(newpixellist) is essential for updating member attribute appropriately.
        ...
        for rowlist in newpixellist:
            col = 0
            for pixel in rowlist:
                FirstColor = newpixellist[row][col][0]
                SecondColor = newpixellist[row][col][1]
                ThirdColor = newpixellist[row][col][2]
                newpixellist[row][col][0] = ThirdColor   #loop at level base!!!!!!!!!!!
                newpixellist[row][col][1] = SecondColor
                newpixellist[row][col][2] = FirstColor
                col += 1
            row += 1
        print(self.outasciifile+ " output file turned to a flip horizontally verion of itself.")
        self.PPM_updatefrompixellist(newpixellist) # This call will update all member apttributes appropriately.

    def PPM_Filp_Brightness(self):
        ''''flips image colors'''
        newpixellist=self.pixellist
        # Hint: What needs to be done here is to convert newpixellist to the equivalent horizontally flipped image.
        # The final call to self.PPM_updatefrompixellist(newpixellist) is essential for updating member attribute appropriately.
        ...
        for rowlist in newpixellist:
            col = 0
            for pixel in rowlist:
                Zero = abs(int(newpixellist[row][col][0]) - 255)
                One = abs(int(newpixellist[row][col][1]) - 255)
                Two = abs(int(newpixellist[row][col][2]) - 255)
                newpixellist[row][col][0] = Zero
                newpixellist[row][col][1] = One
                newpixellist[row][col][2] = Two
                col += 1
            row += 1
        print(self.outasciifile+ " output file turned to a flip horizontally verion of itself.")
        self.PPM_updatefrompixellist(newpixellist) # This call will update all member apttributes appropriately.
