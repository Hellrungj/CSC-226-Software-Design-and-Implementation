...
def printturtle(A, barcode, X, Y):
    ...
    index1 = 0
    print Barcode
    while index1 < len(Barcode): #a while loop and says if the length of Barcode
        if int(index1) % 2 == 0: # make the code only print guard and center bars
            for a in Barcode[index1]: #makes the barcodes for guard and manfure code and center bars
                if int(a) == 1:
                    barcode.right(90)
                    barcode.forward(70)
                    barcode.right(180)
            ...
            for a in Barcode[index1]: #makes the barcodes for product and manfure code
                if int(a) == 1:
                    barcode.right(90)
                    barcode.forward(50)
                    barcode.right(180)
